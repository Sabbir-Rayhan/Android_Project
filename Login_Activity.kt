...
import com.example.mplcricket.gtestrclasses.UserDataClass
...
class Login_Activity : AppCompatActivity() {
    ...
    lateinit var go_to_sign_up_page: TextView
    lateinit var login_login_button: Button
    lateinit var login_forgot_password: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        login_login_button.setOnClickListener{
            Snackbar.make(findViewById(R.id.login_layout), "Wait...........", Snackbar.LENGTH_INDEFINITE).show()
    ...
    private fun login(email: String, password: String) {
        if (email.length in 1..50 && password.length in 6..16){
            ...
            mAuth?.signInWithEmailAndPassword(email, password)
                ?.addOnCompleteListener(this) { task ->
                    if (task.isSuccessful) {
                        ...
                        Snackbar.make(findViewById(R.id.login_layout), "{"+task.exception?.message+"}", Snackbar.LENGTH_LONG).show()
                        updateUI(null)
                    }
            ...
            Snackbar.make(findViewById(R.id.login_layout), "email password can't be empty", Snackbar.LENGTH_SHORT).show()
        }
        ...
        if (uid==null){
            ...
            var valueEventListener= object : ValueEventListener {
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    ...
                    if (post?.email==null){
                        mAuth?.signOut()
                        Snackbar.make(findViewById(R.id.login_layout),"thre is technical issue please connect to our technician", Snackbar.LENGTH_LONG).show()
                    }
                    else{
